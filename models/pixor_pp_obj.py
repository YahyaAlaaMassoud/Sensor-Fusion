...
def create_pixor_pp_obj(input_shape=(800, 700, 35), downsample_factor=4, kernel_regularizer=None, data_format=None):
    # K.clear_session()
...
#     concat = Concatenate(axis=-1)([block1_out, block2_out, block3_out, block4_out])
    ...
    with tf.name_scope("Header"):
        ...
        objectness_map = Conv2D(CLASS_CHANNELS, kernel_size=KERNEL_SIZE, padding=PADDING, kernel_regularizer=KERNEL_REG, activation='sigmoid', name='output_map')(x)

    model = Model(inp, objectness_map)
    return model
