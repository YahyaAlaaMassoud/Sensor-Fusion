...
from .blocks import conv_block, create_sep_conv_block, create_res_conv_block, create_res_sep_conv_block
...
def create_new_det(input_shape=(800, 700, 35), 
    ...
    with tf.name_scope("Backbone"):
        with tf.name_scope("Block1"):
            ...
            x = ReLU()(x)
            x = conv_block(x, filters=FILTERS // 8, kernel_size=KERNEL_SIZE['Block1'], num_layers=1)
            block1_out = x
            # Downsize twice
        ...
        with tf.name_scope("Block2"):
            ...
            x = BatchNormalization()(x)
            x = ReLU()(x)
            x = conv_block(x, filters=FILTERS // 4, kernel_size=KERNEL_SIZE['Block2'], num_layers=1)
            block2_out = x
            # Downsize once
        ...
        with tf.name_scope("Block3"):
            ...
            x = BatchNormalization()(x)
            x = ReLU()(x)
            x = conv_block(x, filters=FILTERS // 2, kernel_size=KERNEL_SIZE['Block3'], num_layers=2)
            block3_out = x
            # change number of channels from X -> BiFPN channels
        ...
        with tf.name_scope("Block4"):
            ...
            x = BatchNormalization()(x)
            x = ReLU()(x)
            x = conv_block(x, filters=FILTERS // 1, kernel_size=KERNEL_SIZE['Block4'], num_layers=5)
            block4_out = x
            # upsmaple once
    ...
    out1, out2, out3, out4 = build_BiFPN_v2(block1_out, block2_out, block3_out, block4_out, filters=BiFPN_filters)
    out5, out6, out7, out8 = build_BiFPN_v2(block1_out, block2_out, block3_out, block4_out, filters=BiFPN_filters)

    with tf.name_scope("FinalOutput"):
        
        out1 = Add()([out1, out2, out3, out4])
        out1 = create_conv_block(out1, filters=BiFPN_filters, kernel_size=3, num_layers=1)
        out2 = Add()([out5, out6, out7, out8])
        out2 = create_conv_block(out2, filters=BiFPN_filters, kernel_size=3, num_layers=1)
        
        concat = Add()([out1, out2])
        concat = create_conv_block(concat, filters=BiFPN_filters, kernel_size=3, num_layers=1)
        
        branch_1 = create_conv_block(concat, filters=BiFPN_filters, kernel_size=3, num_layers=2)
        branch_2 = create_conv_block(concat, filters=BiFPN_filters, kernel_size=3, num_layers=2)
        
        obj        = Conv2D(CLASS_CHANNELS, kernel_size=OUT_KERNEL_SIZE, padding=PADDING, activation='sigmoid', name='objectness_map')(branch_1)
        geo        = Conv2D(REGRESS_CHANNELS, kernel_size=OUT_KERNEL_SIZE, padding=PADDING, name='geometric_map')(branch_2)
        out        = Concatenate(axis=-1, name='output_map')([obj, geo])

    model = Model(inp, out)
    return model

# model = create_new_det()
# model.summary()
