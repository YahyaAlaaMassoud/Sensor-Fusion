...
def sep_conv_block(x, filters, kernel_size, padding):
  x = SeparableConv2D(filters=filters, kernel_size=kernel_size, padding=padding)(x)
  x = BatchNormalization()(x)
  x = ReLU()(x)
  return x
...
def create_pixor_det(input_shape=(800, 700, 35), 
    ...
    with tf.name_scope("Backbone"):
        ...
        with tf.name_scope("Block2"):
            ...
            block2_out = x
            block2_out = Conv2D(filters=BiFPN_filters, kernel_size=1, padding=PADDING)(block2_out)
        ...
        with tf.name_scope("Block3"):
            ...
            block3_out = x
            block3_out = Conv2D(filters=BiFPN_filters, kernel_size=1, padding=PADDING)(block3_out)
        ...
        with tf.name_scope("Block4"):
            ...
            block4_out = x
            block4_out = Conv2D(filters=BiFPN_filters, kernel_size=1, padding=PADDING)(block4_out)
    ...
    out2, out3, out4 = build_BiFPN(block2_out, block3_out, block4_out, filters=BiFPN_filters)
    ...
    with tf.name_scope("FinalOutput"):
      ...
      concat = Conv2D(filters=128, kernel_size=1, padding=PADDING)(concat)
