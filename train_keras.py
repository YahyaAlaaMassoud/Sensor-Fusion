...
from data_utils.generator import KITTIGen, Generator
...
callbacks = [
    tf.keras.callbacks.TensorBoard(log_dir=logdir, histogram_freq=0, write_graph=True, write_images=False, update_freq='batch', profile_batch=2)
]

data_len = int(np.ceil(len(configs['chosen_ids']) / BATCH_SIZE))

model.fit_generator(generator=train_gen, steps_per_epoch=data_len, epochs=100, callbacks=callbacks, verbose=True, use_multiprocessing=False)
...
#     progress = data_len // 10
#     cur_progress = progress

#     print('Total number of batches is ->', data_len)
...
#     #     # Fetch batch
#         encoded_pcs, (encoded_obj, encoded_boxes) = batch
...
#         # start = datetime.now()
#         # metrics = model.train_on_batch(x=encoded_pcs, y=[encoded_obj, encoded_boxes])
#         metrics = model.fit(x=encoded_pcs, y=[encoded_obj, encoded_boxes], batch_size=BATCH_SIZE, epochs=1, verbose=False, workers=1, use_multiprocessing=False)
...
#         del encoded_boxes, encoded_obj, encoded_pcs
