...
def compute_loss(model, x, y, training):
    ...
    geo_loss = losses['geo_map'](y[1], geo) * 2.0
...
for cur_epoch in range(1, EPOCHS):
    ...
    for batch in train_gen:
        ...
        # start = datetime.now()
        loss_value, grads = compute_grads(model, pc, (obj, geo))
        ...
        if train_steps % configs['vis_every'] is 0 and train_steps is not 0:
            for chosen_id in configs['chosen_ids']:
                ...
                obj_mask = np.expand_dims(np.expand_dims(np.squeeze(obj_mask), axis=0), axis=-1)
                with img_writer.as_default():
                    tf.summary.image('{}_obj_map'.format(chosen_id), obj_map, step=train_steps)
                    tf.summary.image('{}_obj_mask'.format(chosen_id), obj_mask, step=train_steps)
                ...
                with img_writer.as_default():
                    tf.summary.image('{}_plot'.format(chosen_id), plot, step=train_steps)
                    tf.summary.image('{}_obj_pred'.format(chosen_id), obj, step=train_steps)
